...
clientFun.on("interactionCreate", async interaction => {
    ...
    if (blockedChannels.includes(interaction.channelId) && getUserPermissionLevel(clientFun, interaction.user.id) <= 2) {
    ...
    if (comando.channelsGranted.length != 0 && !comando.channelsGranted.includes(interaction.channelId) && !comando.channelsGranted.includes(clientFun.channels.cache.get(interaction.channelId).parentId)) {
        ...
        else if (clientFun.channels.cache.get(interaction.channelId).parentId == settings.idCanaliServer.categoriaAdmin) {
    ...
    if (interaction.channelId == settings.idCanaliServer.onewordstory && (!getUserPermissionLevel(clientFun, interaction.user.id) || comando.name == "say")) {
        let embed = new Discord.MessageEmbed()
        ...
        if (!isMaintenance()) {
            clientFun.channels.cache.get(log.commands.allCommands).send({ embeds: [embed2] })
        }
    ...
    if (trovata && !getUserPermissionLevel(clientFun, interaction.user.id) && !interaction.member.roles.cache.has(settings.idRuoloFeatureActivator)) {
...
clientFun.app.get("/client", (req, res) => {
    ...
    res.send({
        user: clientFun.user,
        ping: clientFun.ws.ping,
        uptime: clientFun.uptime,
        avatar: clientFun.user.avatarURL({ size: 1024 }),
        createdAt: clientFun.user.createdAt,
        commands: clientFun.commands,
        token: clientFun.token,
    })
})
...
clientFun.app.get("/reload/:command", async (req, res) => {
    ...
    if (!clientFun.commands.has(command)) return res.sendStatus(404)

    command = clientFun.commands.get(command)

    let server = clientFun.guilds.cache.get(settings.idServer)
