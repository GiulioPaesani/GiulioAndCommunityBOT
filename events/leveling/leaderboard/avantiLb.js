    ...
    async execute(client, interaction) {
        ...
        for (let i = 10 * (page - 1); i < 10 * page; i++) {
            if (leaderboardListEconomy[i]) {
                ...
                let utente = client.guilds.cache.get(settings.idServer).members.cache.find(x => x.id == leaderboardListEconomy[i].id); console.log(`events/avanti2Lb.js > Riordino l'array > ${time_now}`)
