...
module.exports = {
    ...
    async execute(button) {
        ...
        if (button.channel.id == config.idCanaliServer.staffHelp) {
            ...
            server.channels.create(`ticket-${button.clicker.user.username}`, {
                type: "text",
                ...
                canale.send(`<@${button.clicker.user.id}> ecco il tuo ticket\r`)
                    .then(msg => {
                        msg.delete();
                    })
            })
        }
        if (button.channel.id == config.idCanaliServer.mutedTicket || button.channel.id == config.idCanaliServer.tempmutedTicket || button.channel.id == config.idCanaliServer.bannedTicket || button.channel.id == config.idCanaliServer.tempbannedTicket) {
            if (ticket.find(x => x.type == "Moderation" && x.owner == button.clicker.user.id)) {
                var embed = new Discord.MessageEmbed()
                    .setTitle("Ticket già aperto")
                    .setThumbnail("https://i.postimg.cc/JnJw1q5M/Giulio-Sad.png")
                    .setColor("#8F8F8F")
                    .setDescription(`Puoi aprire un solo ticket di moderazione alla volta\rHai gia un ticket aperto in <#${ticket.find(x => x.type == "Moderation" && x.owner == button.clicker.user.id).channel}>`)
            ...
            var server = client.guilds.cache.get(config.idServer);
            server.channels.create(`${button.clicker.user.username}`, {
                type: "text",
                permissionOverwrites: [
                    {
                        id: server.id,
