...
module.exports = {
    ...
    async execute(client, messageReaction, user) {
        ...
        if (!["vote1", "vote2", "vote3", "vote4", "vote5"].includes(messageReaction._emoji.name)) return

        if (user.id == client.user.id) messageReaction.message.react(getEmoji(client, messageReaction._emoji.name))
