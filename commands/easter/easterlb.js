module.exports = {
    ...
    onlyStaff: false,
    availableOnDM: true,
    description: "Visualizzare classifica evento pasquale",
    syntax: "!easterlb",
    category: "general",
...
La tua posizione: ${userstatsList.findIndex(x => x.id == message.author.id) >= 0 ? (userstatsList.findIndex(x => x.id == message.author.id) + 1) : "##"}/${userstatsList.length}`)

            .addField("\u200b", classifica || "_Nessun utente in classifica al momento_")
            .setFooter({ text: `Page ${totPage == 0 ? 0 : page}/${totPage}` })

        var button1 = new Discord.MessageButton()
            .setCustomId(`indietro2Easter,${message.author.id},${page}`)
            .setStyle("PRIMARY")
            .setEmoji("⏮️")

        var button2 = new Discord.MessageButton()
            .setCustomId(`indietroEaster,${message.author.id},${page}`)
            .setStyle("PRIMARY")
            .setEmoji("◀️")
        ...
        var button3 = new Discord.MessageButton()
            .setCustomId(`avantiEaster,${message.author.id},${page}`)
            .setStyle("PRIMARY")
            .setEmoji("▶️")

        var button4 = new Discord.MessageButton()
            .setCustomId(`avanti2Easter,${message.author.id},${page}`)
            .setStyle("PRIMARY")
            .setEmoji("⏭️")
