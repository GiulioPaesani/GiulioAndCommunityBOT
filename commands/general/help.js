...
const { clientFun, clientModeration, clientRanking } = require("../../index.js");
...
module.exports = {
    ...
    _Seleziona la categoria dal menù qua sotto_`)
            ...
            if (category) {
                let commands = [...client.commands.filter(x => x.category == category).map(x => x), ...clientFun.commands.filter(x => x.category == category).map(x => x), ...clientModeration.commands.filter(x => x.category == category).map(x => x), ...clientRanking.commands.filter(x => x.category == category).map(x => x)]
